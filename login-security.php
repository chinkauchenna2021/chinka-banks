...
if(isset($_POST['log_sec_btn'])){
 $logemail =  $_SESSION['log_email'];
  $logpass = $_SESSION['log_pass'];
   $logcpass = $_SESSION['log_cpass'];
    $logsec = $_POST['log_sec'];
    $actives;

    if(isset($logemail) && isset($logpass) && isset($logcpass)){

       if(!empty($logsec) && (strlen($logpass) == strlen($logcpass))){
          ...
          if($query){
               if(mysqli_num_rows($query) >= 1){
                  echo "welcome Back";
                  $_SESSION['back'] = 'welcome-back';
                  $_SESSION['log_sec'] = $logsec;
                ...
                //  echo "<script>alert($actives)</script>";
                 if($actives == 1){
                     ...
                     echo "<script>window.open('registered-users.php','_self')</script>";

                 }else{

                  unset($_SESSION['id']);
                  unset($_SESSION['log_sec']);
                  unset($_SESSION['back']);
                  echo "<script>window.open('suspension.php','_self')</script>";
                 }
        ...
        echo "Please ensure to fill the input correctly";
       }
                                ...
                                <div class="tabs-custom tabs-horizontal tabs-line" id="tabs-1">
                                    ...
                                    <div class="tab-content">
                                        <div class="tab-pane fade show active" id="tabs-1-1">
                                            <h3>Eight Digit Security Code</h3>
                                            <!-- RD Mailform-->
                                            <form class="rd-form form-lg" action="login-security.php" method="POST">
                                                <div class="form-wrap">
                                                    <input class="form-input" id="login-password" type="password" name="log_sec" data-constraints="@Required">
                                                    <label class="form-label" for="login-password">eg ********</label>
                                                </div>
                                                <button class="button button-lg button-round button-block button-primary" name="log_sec_btn" type="submit">Verify</button>
                                            </form>
